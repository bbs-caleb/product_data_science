...
def how_much_to_order(request_data: SKURequest) -> dict:
    ...
    try:
        ...
        pred_col = f"pred_{horizon_days}d_q{int(confidence_level * 100)}"

        # Get the forecast for the given SKU
        sku_row = predictions[predictions["sku_id"] == sku_id]
        forecast = sku_row[pred_col].values[0]

        # Calculate how much to order: ceil(max(forecast - current_stock, 0))
        import math
        recommended = int(math.ceil(max(0, forecast - current_stock)))
...
def stock_level_forecast(request_data: SKURequest) -> dict:
    ...
    try:
        ...
        # Calculate stock level: max(current_stock - forecast, 0)
        stock_level = max(0, current_stock - forecast)
    ...
    try:
        ...
        for sku_info in skus:
            ...
            if len(sku_row) > 0:
                ...
                # SKU is in low stock if current_stock < forecast
