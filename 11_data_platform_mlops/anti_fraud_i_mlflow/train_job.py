...
def recall_at_precision(
    ...
    # Найти максимальный recall при precision >= min_precision
    # precision_recall_curve возвращает массивы в порядке убывания threshold
    # поэтому precision растёт, а recall падает
...
def recall_at_specificity(
    ...
    # specificity = 1 - FPR, поэтому FPR <= 1 - min_specificity
    # Нам нужен максимальный TPR (recall) при specificity >= min_specificity
...
def job(
    ...
    # Определение признаков (все колонки кроме target)
    ...
    # Предсказания (отрицательный score_samples, т.к. аномалии имеют меньший score)
