...
product_weekly as (select product_name,
...
-- расчет лагов для каждого продукта
product_lags as (select product_name,
                        monday,
                        max_price,
                        y,
                 ...
                 from product_weekly window w as (partition by product_name order by monday rows between unbounded preceding and unbounded following)
    ),

-- расчет фичей для каждого продукта (avg/max/min на основе лагов)
    product_features as (
select
    product_name,
    monday, max_price,
...
-- расчет глобальных фичей на основе сумм лагов
    global_features as (
select
    monday,
    y_all_lag_1,
